...
import { View, TouchableOpacity, Text, StyleSheet, Image, Platform, StatusBar } from 'react-native';
    ...
    return (
      <View style={styles.container}>
        <ScrollView
          ...
          style={{ paddingHorizontal: helpers.size(20) }} contentContainerStyle={locale == "ar" && { alignItems: 'flex-end' }}>
          <View style={[{
            flexDirection: 'row',
            width: '100%',
            alignItems: 'center',
          ...
          locale == "ar" && { flexDirection: 'row-reverse' }]}>
            ...
            <View style={{ paddingLeft: wp(3), paddingRight: wp(3), justifyContent: 'center' }}>
              <Text style={[styles.title, locale == "ar" && { textAlign: 'right' }]}>{name}</Text>
              <TransText style={[styles.subtitle, locale == "ar" && { textAlign: 'right' }]} transkey="EDIT_PROFILE" />
            </View>
          </View>
          <TouchableOpacity activeOpacity={0.8} style={[styles.menuItem, locale == "ar" && { flexDirection: 'row-reverse' }]}
            onPress={() => {
            ...
            <TransText style={styles.menuItemText} transkey={token ? "MENU_SIGNOUT" : "MENU_SIGNIN"} />
          </TouchableOpacity>
          <TouchableOpacity activeOpacity={0.8} style={[styles.menuItem, locale == "ar" && { flexDirection: 'row-reverse' }]}
            onPress={() => {
              drawer.current.close()
          ...
          <TouchableOpacity activeOpacity={0.8} style={[styles.menuItem, locale == "ar" && { flexDirection: 'row-reverse' }]}
            ...
            <Notification_icon width={helpers.size(20)} height={helpers.size(20)} />
            <TransText style={styles.menuItemText} transkey="MENU_NOTIFICATION" />
          </TouchableOpacity>
          <TouchableOpacity activeOpacity={0.8} style={[styles.menuItem, locale == "ar" && { flexDirection: 'row-reverse' }]}
            onPress={() => {
              drawer.current.close()
          ...
          <TouchableOpacity activeOpacity={0.8} style={[styles.menuItem, locale == "ar" && { flexDirection: 'row-reverse' }]}
            ...
            <Change_lang_icon width={helpers.size(20)} height={helpers.size(20)} />
            <TransText style={styles.menuItemText} transkey="MENU_CHANGE_LANG" />
          </TouchableOpacity>
          <TouchableOpacity activeOpacity={0.8} style={[styles.menuItem, locale == "ar" && { flexDirection: 'row-reverse' }]}
            onPress={() => {
              drawer.current.close()
